...
def process_pair(x, y, d1, d2):
    with open('graph.pkl', 'rb') as f:
        G_local = pickle.load(f)
    try:
        result = maximizer1(G_local, x, y, d1, d2)
...
with ProcessPoolExecutor(max_workers=num_cores) as executor:
    futures = {executor.submit(process_pair, x, y, d1, d2): (x, y, d1, d2) for x, y, d1, d2 in tasks}
...
print(f"Maximizer dictionary saved to maximizer_dict1.pkl in {total_time:.2f} seconds")
